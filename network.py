...
class Real_SRNet(keras.Model):
    def __init__(self,channel:int=64,block_num:int=16,scale:int=2,kernel_size:tuple[int,int]=(3,3),activation=tf.nn.leaky_relu) -> None:
        super().__init__()
        self.conv1=layers.Conv2D(channel,kernel_size,padding="same",activation=None,dtype="float32")
        self.block1=EDSR_Block(channel,kernel_size,activation)

        self.blocks=[EDSR_Block(channel,kernel_size,activation) for _ in range(block_num-1)]
